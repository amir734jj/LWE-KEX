...
def initialize():
    raw_basis = Matrix([[1 if i == j else 0 for j in range(sub_dimension)] if i < sub_dimension -1 else [1/2 for i in range(sub_dimension)] for i in range(sub_dimension)])
    correct_basis = Matrix(raw_basis).echelon_form()  # convert (1/2, 1/2, ..., 1/2) to (0, 0, ..., 1)
     
    integer_lattice = IntegerLattice(correct_basis)
    
    main_polyhedron = calculate_voronoi_cell(raw_basis)
